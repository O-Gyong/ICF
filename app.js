...
function loadState() {
  ...
  if (localRoster) {
    ...
    // If the roster is the old mock roster (first player is 선수2, number 11), overwrite it
    if (parsed.length > 0 && parsed[0].name === '선수2' && parsed[0].number === '11') {
  ...
  if (localSchedule) {
    ...
    if (parsed.length > 0 && (parsed[0].opponent === '화이어볼즈 BB' || !parsed.find(s => s.opponent === 'BDR 스타터스 리그' && s.participants.includes('선수1')))) {
...
const pageTitles = {
  ...
  roster: { title: '팀 로스터', subtitle: '우리 팀원 목록 및 주요 포지션 관리' }
};
